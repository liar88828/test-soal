...
import { soalListCheckLoader } from "@/action/soal.action";
...
export function AnswerCheck() {
	const { answers, list } = useLoaderData<LoaderProps<typeof soalListCheckLoader>>()
					...
					<Card key={q.id} className="mb-4">
						...
						<CardContent>
							...
							{["A", "B", "C", "D", "E"].map((opt) => {
								...
								const variant: Variant = match({ isSelected, isCorrect })
									.with({ isSelected: true, isCorrect: true }, () => "default" as Variant)
									.with({ isSelected: true, isCorrect: false }, () => "destructive" as Variant)
									.otherwise(() => "outline" as Variant);
